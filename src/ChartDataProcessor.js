...
export class ChartDataProcessor {
  ...
  setup({ vm, type }) {
    ...
    if (typeof vm.maxV === 'undefined') {
      ...
      let sep2 = sep; if (sep2 === 0) sep2 = 100; if (sep2 < 100) {
        while (sep2 < 100) { emp *= 10; sep2 = sep * emp; } //获取三位精度的刻度间隔
      ...
      console.log('vm.min, vm.max', vm.min, vm.max)
    }
    ...
    vm.MarkTexts = vm.Marks.map((mc, index) => { return ((vm.min + vm.sep * (index + 1)) / vm.emp).toString() });//刻度文字
